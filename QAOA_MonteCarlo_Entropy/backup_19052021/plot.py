...
def plot_adiabatic_instantaneous_eigenstates(t, t_evals, image_name):
    ...
    for i in range(len(t_evals)):
        if i == 0:
            plt.plot(t, t_evals[i], label=r"$E_{min} ($t = "+str(t[len(t)-1])+"$) = $"+str(round(t_evals[i][len(t)-1],2)))
        elif i == len(t_evals)-1:
            plt.plot(t, t_evals[i], label=r"$E_{max} ($t = "+str(t[len(t)-1])+"$) = $"+str(round(t_evals[i][len(t)-1],2)))
        else:
...
def plot_energies_entropies(axes, i, j, M, value_mean, value_std, p, quantity, error_bars, loc_legend, y_label, alpha_trans):
    if error_bars == "error":
        for n in range(len(p)):
            axes[i,j].plot(np.arange(M+1), value_mean[n], label=r"$p = $"+str(p[n])+", $M = $"+str(M)+quantity+str(round(value_mean[n,M-1],2)))
...
def plot_monte_carlo_simulation_results(exp_values_1, exp_values_1_std, exp_values_2, exp_values_2_std, entropy_1, entropy_1_std, entropy_2, entropy_2_std, fidelity_1, fidelity_1_std, fidelity_2, fidelity_2_std, error_bars, p_array, M_1, M_2, opt_approach, image_name):
    ...
    axs = plot_energies_entropies(axs, 0, 0, M_1, exp_values_1, exp_values_1_std, p_array, 
                                  h_p_string, error_bars, "lower right", r"$\langle H_p \rangle$", alpha_trans)
    ...
    axs = plot_energies_entropies(axs, 0, 1, M_1, entropy_1, entropy_1_std, p_array, 
                                  s_p_string_1, error_bars, "upper right", r"$S(|\alpha_i|^2)$", alpha_trans)
    ...
    axs = plot_energies_entropies(axs, 1, 1, M_2, entropy_2, entropy_2_std, p_array, 
                                  s_p_string_2, error_bars, "upper right", r"$S(|\alpha_i|^2)$", alpha_trans)
    ...
    axs = plot_energies_entropies(axs, 0, 2, M_1, fidelity_1, fidelity_1_std, p_array, 
                                  f_p_string, error_bars, "lower right", r"$F = |\langle \gamma,\beta |\phi_0\rangle|$", alpha_trans)
...
def plot_prob_amplitudes(state_probs_1, state_probs_1_std, state_probs_2, state_probs_2_std, p_array, M_1, M_2, L, rand_approach, opt_approach, image_name, y_max):
